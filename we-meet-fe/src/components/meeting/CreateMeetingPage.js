...
const CreateMeetingPage = () => {
  ...
  const isDateSelected = (date) => {
    return meetingData.selectedDates.some(selectedDate => 
  ...
  const toggleDateSelection = (date) => {
    ...
    setMeetingData(prev => {
      ...
      return {
        ...prev,
        selectedDates: dateExists
          ? prev.selectedDates.filter(d => d.toDateString() !== date.toDateString())
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!meetingData.title || meetingData.selectedDates.length === 0) {
      setShowAlert(true);
      return;
    }
    ...
    <div className="max-w-lg mx-auto p-4">
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="space-y-4">
          ...
          {meetingData.selectedDates.length > 0 && (
            <div className="text-right text-sm text-gray-600">
              선택된 날짜: {meetingData.selectedDates.length}일
            </div>
          )}
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center space-x-4 bg-gray-50 p-4 rounded-lg">
            ...
            <input
              type="time"
              name="timeRange.end"
              value={meetingData.timeRange.end}
              onChange={(e) => setMeetingData(prev => ({
                ...prev,
                timeRange: { ...prev.timeRange, end: e.target.value }
              }))}
              min={meetingData.timeRange.start}
              max="21:00"
              className="p-2 rounded border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none"
            />
          </div>
